...
function getDatabase() {
  if (pool) {
    // Return a wrapper that provides SQLite-compatible API
    return createDatabaseWrapper(pool);
  }
  ...
  // Create pool immediately (will be ready after initialization)
  // This allows synchronous use, but queries may wait for initialization
  const config = {
    host: process.env.MYSQL_HOST || 'localhost',
    port: parseInt(process.env.MYSQL_PORT || '3306'),
    user: process.env.MYSQL_USER || 'root',
  ...
  // This is a simple blocking approach - in production you might want non-blocking
  if (dbInitPromise) {
