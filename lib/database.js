...
function getDatabase() {
  ...
  db = new sqlite3.Database(dbPath, (err) => {
    ...
    db.get('SELECT COUNT(*) as count FROM menu_categories', [], (err, result) => {
      if (!err && result) {
        if (result.count === 0) {
          console.warn('⚠️  WARNING: Database is empty (0 categories found)!');
          console.warn('   Please run: node scripts/seed-menu.js');
        } else {
          console.log(`✅ Database has ${result.count} categories`);
        }
      }
    });
