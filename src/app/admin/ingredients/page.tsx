...
export default function AdminIngredients() {
    ...
    try {
      ...
      const newConfigs = [
        ...currentConfigs.map((c: any) => ({
          ...
          ingredient_group: c.ingredient_group ?? null,
          volume_prices: null,
        })),
        {
  ...
  return (
    <div className="space-y-6" dir={language}>
      ...
      {/* Category Configurations Section */}
      <Card>
        <CardHeader>
          <CardTitle>{t('Category Configurations')}</CardTitle>
          <CardDescription>
            {t('Attach ingredients to menu categories and group them. All items in a category will have access to these ingredients.')}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {categories.length === 0 ? (
            <div className="text-center py-8 text-slate-500">
              <p className="text-sm">{t('No categories found.')}</p>
              <p className="text-xs mt-2">{t('Create categories in the Menu section first.')}</p>
            </div>
          ) : (
              ...
              {categories.map((category) => (
                ...
                >
                  <div className="flex-1">
                    <h3 className="font-medium text-slate-900">{t(category.name)}</h3>
                    {category.description && (
                      <p className="text-xs text-slate-500 mt-1">{t(category.description)}</p>
                    )}
                  </div>
                  <Link href={`/admin/ingredients/configure/${category.id}`}>
                    <Button
                      variant="outline"
                      size="sm"
                      className="ml-4 bg-purple-600 hover:bg-purple-700 text-white border-purple-600"
                    >
