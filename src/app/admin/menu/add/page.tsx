  ...
  async function fetchCategories() {
    try {
      ...
      if (data.categories?.length > 0) {
        ...
        const fallbackId = data.categories[0].id?.toString() || '';
        const foundMatch = data.categories.find((c: Category) => c.id?.toString() === fromUrl);
        setForm((f) => ({
          ...f,
          category_id: (fromUrl && foundMatch ? fromUrl : fallbackId) || '',
        }));
      }
    } catch (error) {
