 ...
 * @param pdfAttachment - Optional PDF receipt from PayPlus
 */
export async function sendOrderConfirmationEmail(orderData: OrderEmailData, pdfAttachment?: Buffer): Promise<boolean> {
  try {
    ...
    const generateItemHtml = (item: any) => {
      ...
      if (item.customIngredients && item.customIngredients.length > 0) {
        const ingredientsList = item.customIngredients.map((ing: any) =>
      ...
      if (item.additionalItems && item.additionalItems.length > 0) {
        const additionalList = item.additionalItems.map((add: any) =>
    ...
    const itemsList = orderData.items
      .map(item => {
        ...
        if ((item as any).customIngredients && (item as any).customIngredients.length > 0) {
          const ingList = (item as any).customIngredients.map((ing: any) => `${ing.name} (+₪${ing.price.toFixed(2)})`).join(', ');
          text += `\n  מרכיבים מותאמים: ${ingList}`;
        }
        if ((item as any).additionalItems && (item as any).additionalItems.length > 0) {
          const addList = (item as any).additionalItems.map((add: any) => `${add.name} (+₪${add.price.toFixed(2)})`).join(', ');
          text += `\n  תוספות: ${addList}`;
        }
        return text;
      })
