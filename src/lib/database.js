...
async function initializeTables(pool) {
  try {
    ...
    // Migrate status column from TEXT to VARCHAR if needed
    await migrateStatusColumn(pool, 'orders');
    ...
    // Migrate status column from TEXT to VARCHAR if needed
    await migrateStatusColumn(pool, 'order_telegram_notifications');
...
async function migrateStatusColumn(pool, tableName) {
  try {
    ...
    const [columns] = await pool.query(`SHOW COLUMNS FROM ?? WHERE Field = ?`, [tableName, 'status']);
    if (columns.length > 0) {
      const column = columns[0];
      // If column type is TEXT, migrate to VARCHAR(50)
      if (column.Type && column.Type.toUpperCase().includes('TEXT')) {
        await pool.query(`ALTER TABLE ?? MODIFY COLUMN status VARCHAR(50) DEFAULT 'pending'`, [tableName]);
        console.log(`Migrated status column in ${tableName} from TEXT to VARCHAR(50)`);
      }
    }
  } catch (err) {
    // Ignore errors if table doesn't exist or column doesn't exist
