  ...
  try {
    ...
    return new Promise<NextResponse>((resolve) => {
      db.all('SELECT * FROM locations ORDER BY sort_order, id', [], (err: Error | null, rows: any[]) => {
        ...
        const translatedLocations = (rows || []).map(location => translateObject(location));
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { country, city, address, hours, phone, email, image, map_url, is_active, sort_order } = body;
    ...
    return new Promise<NextResponse>((resolve) => {
      db.run(
        `INSERT INTO locations (country, city, address, hours, phone, email, image, map_url, is_active, sort_order) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [country, city, address, hours || '', phone || '', email || '', image || '', map_url || '', is_active !== false ? 1 : 0, sort_order || 0],
        function (this: any, err: Error | null) {
          ...
          resolve(
            NextResponse.json(
              { id: this.lastID, country, city, address, hours, phone, email, image, map_url },
              { status: 201 }
            )
          );
        }
